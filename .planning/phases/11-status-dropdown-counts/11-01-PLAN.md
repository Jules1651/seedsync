...
must_haves:
  truths:
    ...
    - "Counts update when files change status (via existing subscription)"
  ...
  key_links:
    - from: "file-options.component.ts"
      ...
      via: "subscription computes statusCounts Map"
      pattern: "statusCounts.*computeStatusCounts"
...
<task type="auto">
  <name>Task 1: Add count computation and formatting to TypeScript</name>
  <files>src/angular/src/app/pages/files/file-options.component.ts</files>
  <action>
Modify file-options.component.ts to add status count tracking:
...
6. Update the existing `ngOnInit` subscription to also call:
   - `this.statusCounts = this.computeStatusCounts(files);`
   - Place before the existing `detectChanges()` call

Note: The existing `isStatusEnabled` pattern becomes redundant (replaced by `getCount(status) > 0`), but keep it for now to minimize changes. Can be cleaned up in future refactor.
  </action>
  <verify>
...
Run `npm test` from src/angular - existing tests should still pass.
  ...
  <done>
TypeScript compiles without errors. Component has statusCounts Map, formatCount() method returns formatted numbers, getCount() returns raw counts.
  </done>
</task>
...
4. Functional verification:
   ...
   - Counts update when files change (e.g., after download completes)
</verification>
...
- UX-03: Counts update in real-time as files change status - SATISFIED (via existing subscription)
